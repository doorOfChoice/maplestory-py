...
PLAYER_STEP_UP = 36.0
# 近垂直 foothold 判定阈值：|dy| >= 此倍数 × |dx| 的斜段视为墙（不贴坡/不落地、
# 不参与台阶豁免）。地图常把立面误写成 1~7px 宽的极陡斜段，当坡走会单帧垂直瞬移。
# 普通台阶斜率为 1.9，取 3.0 只吃掉真正的立面。
FOOTHOLD_WALL_SLOPE = 3.0
# 出生点贴地吸附容差：WZ portal 标注 y 常落在 foothold 线上下数 px~数十 px，
# 首帧没有穿线信息，落点在容差内取最近面双向吸附，防止一进图就穿地坠落
SPAWN_SNAP_TOL = 60.0
# 瞬移落点吸附容差：水平落点最近可行走地面与脚底的竖直差在此内就贴上去（含上坡
# 防埋），超出则保持腾空、交由重力坠落（落点下方无地面时则原地不动）
TELEPORT_SNAP_TOL = 40.0
...
MESO_ICON_TIERS = ((50, 0), (100, 1), (1000, 2), (float("inf"), 3))
...
# 强化卷轴（官方 204xxxx 物品；登记表见 game/systems/scrolls.py）
SCROLL_FEE_BASE = 500            # 卷轴强化费基础（金币）
SCROLL_FEE_PER_LEVEL = 200       # 卷轴强化费每级增量（金币）
# 缺 price 字段的物品兜底买价（WZ 有 price 优先；键用补零 8 位 id）
# 官方卷轴 WZ info.price 恒为 1，故商店价一律由脚本/FALLBACK_PRICES 决定。
FALLBACK_PRICES = {
    "02043001": 50000,           # 单手剑攻击卷轴 60%
    "02043005": 150000,          # 单手剑攻击卷轴 30%
    "02043003": 200000,          # 单手剑攻击必成卷
    "02043701": 50000,           # 短杖魔力卷轴 60%
    "02043705": 150000,          # 短杖魔力卷轴 30%
    "02043703": 200000,          # 短杖魔力必成卷
    "02044501": 50000,           # 弓攻击卷轴 60%
    "02044505": 150000,          # 弓攻击卷轴 30%
    "02044503": 200000,          # 弓攻击必成卷
}
...
# 活动类官方任务（季节/联动/庆典）不加载：编号区段黑名单（左闭右开，
# 区段含义见 systems.quests.filter_world_quest_defs 与 Quest.wz 数据普查）。
QUEST_EVENT_RANGES: tuple[tuple[int, int], ...] = (
    (4437, 4438),      # 白色圣诞节
    (4675, 4677),      # 直美的招待（活动 NPC）
    (4697, 4699),      # Missing You / 鬼神名号（活动 NPC）
    (4710, 4716),      # 寻找枫叶之心碎片（周年庆）
    (5009, 5013),      # 交换圣诞礼物盒
    (5041, 5048),      # 蕃茄农场 / 喝醉的怪物（夏日活动）
    (5065, 5069),      # 宝藏猎人（活动 NPC）
    (8248, 8255),      # 枫叶市集 / 养鸡场
    (8552, 8588),      # 新春猜谜 / 鞭炮 / 端午节
    (8600, 8742),      # 周边联动 / 枫彩票 / 年菜 / 字母搜集等
    (8879, 8900),      # 庆祝两周年 / 巨大蕃茄
    (9000, 11000),     # 中秋/圣诞/新年/情人节/万圣节/特殊干员O 等季节活动线
    (28000, 29000),    # 圣诞惊喜舞会
    (52000, 53000),    # 4周年庆典 / 夏日 / 迷你地城入场券
)
# ── 地图通行 / 缓存 ────────────────────────────────────────────────
