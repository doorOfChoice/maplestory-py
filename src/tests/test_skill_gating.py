...
def test_unimplemented_skill_hidden_from_window_and_learn():
    """未实装技能（无 ball/hit/mob/伤害标记）不出现在可学/技能窗，也不可学。"""
    book = SkillBook(None, 2200, defs={
        "2201009": make_def("2201009", mpCon=13, x=5),
        "2201003": make_def("2201003")})
    book.add_sp(220, 5)
    assert book.learnable() == ["2201003"]
    assert book.skills_for_group(220) == ["2201003"]
    assert book.can_learn("2201009", player_level=120) is False
    assert book.learn("2201009", player_level=120) is False


def test_unimplemented_prereq_does_not_block_followup():
    """前置是未实装技能（无施放标记）时不再阻塞后续技能学习。"""
    book = SkillBook(None, 2200, defs={
        "2201009": make_def("2201009", mpCon=13, x=5),
        "2201003": make_def("2201003", req={"2201009": 5})})
    book.add_sp(220, 5)
