...
- 回车/空格 = `confirm`：**两步防误触**——本步有蓝字链接或 yes/no 按钮时，第一次
  只是把键盘焦点落到第一项（金色高亮），再按一次才真正触发；终态步（无链接无
  按钮）一步即等同按 BtOK。
- ↑ / ↓ / Tab = `up` / `down`：焦点在「可见蓝字链接 → yes → no」合成序列上循环。
- 数字键 1~9 = 直选第 N 条可见蓝字链接（一步生效，不经焦点）。
- Esc = `close`：有 `buttons.no` 走 no 分支，否则直接结束。
- 点面板外 → 收起会话；按 E 走近距离开的会话走远（>140px）、切图、重生 → 销毁；
  鼠标远处点 NPC 开的会话不受走远收起影响。
...
- `text`/`label` 由宿主统一做官方标记解析（`render_markup`）：`#t<id>#` 物品名、
  ...
  多段文本仍建议写成多个数组元素。
- **着色**：会话面板分段渲染颜色码 `#r` 红 / `#g` 绿 / `#b` 蓝 / `#d` 橙 / `#k` 回基色；
  实体名解析后**自动包色**（物品→蓝、怪物→红、地图→绿、NPC→橙），无需手写。
  任务日志/气泡等纯文本场景不渲染颜色（标记被剥离）。
